...
public class DeviceImpl implements DeviceService {
    ...
    public Map<Object, Object> getDeviceDetail(TerminalDevice t, Integer user_id) {
        ...
        try {
            ...
            t = deviceMapper.getDeviceDetailById(t.getEquipment_id()); //查询设备详情返回的结果.
            //更换为字段，查询设备是否有正在进行中的售后
            t.setProcessRepair(deviceMapper.repairStatus(t.getEquipment_id()) > 0 ? true : false);//查询维修结果返回bootean的维修状态
...
//			return map;
    ...
    public void insert(TerminalDevice device) {
        ...
        //添加设备状态表
        deviceMapper.insertEquipmentstatus(device.getEquipment_id(), device.getWorkStatus());
        //添加提醒表
        deviceMapper.insertRemind(device.getEquipment_id());
    }
